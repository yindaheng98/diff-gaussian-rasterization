	...
	const bool debug)
{
  ...
  torch::Tensor tran_Ab2d = torch::zeros({P, 6 + 9 + 4 + 9}, means3D.options());
  torch::Tensor tran_alpha = torch::zeros({P}, means3D.options());
  torch::Tensor tran_det = torch::zeros({P}, means3D.options());
  torch::Tensor tran_equations = torch::zeros({P, 3, 7}, means3D.options());
  ...
  {  
	  ...
	  tran_Ab2d.contiguous().data<float>(),
	  tran_alpha.contiguous().data<float>(),
	  tran_det.contiguous().data<float>(),
	  tran_equations.contiguous().data<float>(),
	  antialiasing,
	  debug);
  }

  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, tran_Ab2d, tran_alpha, tran_det, tran_equations, regressionBuffer);
}
